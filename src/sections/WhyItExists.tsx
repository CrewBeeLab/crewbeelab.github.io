...
export const WhyItExists = ({ t }: { t: Copy }) => (
  <section id="why-crewbee" className="py-24 md:py-32 lg:py-48 bg-paper-warm/40 border-y border-ink/5 relative overflow-hidden">
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-100px' }} transition={{ duration: 0.8 }} className="max-w-4xl mx-auto px-6 text-center relative z-10">
      ...
      <p className="text-lg md:text-xl font-serif text-ink/70 leading-relaxed mb-6">{t.why.paragraphs[0]}</p>
      <p className="text-lg md:text-xl font-serif text-ink/70 leading-relaxed mb-8">{t.why.paragraphs[1]}</p>
      <p className="text-xl md:text-2xl font-serif font-bold italic text-ink leading-relaxed">{t.why.emphasis}</p>
    </motion.div>
  </section>
);
