export const homepageContent = {
  en: {
    ...
    installation: {
      ...
      subtitle: 'Set up CrewBee with one command, then open your project and start with the built-in Coding Team.',
      trustPoints: ['No project-local node_modules', 'No manual OpenCode config editing', 'Runs setup verification automatically'],
      commandLabel: 'Recommended',
      command: 'npx crewbee@latest setup --with-opencode',
      commandNote: 'Installs OpenCode if needed, installs CrewBee, and verifies setup.',
      copy: 'Copy',
      copied: 'Copied',
  ...
  zh: {
    ...
    installation: {
      ...
      trustPoints: ['不安装到业务项目 node_modules', '不需要手动修改 OpenCode 配置', '自动完成 setup 校验'],
      commandLabel: '推荐命令',
      command: 'npx crewbee@latest setup --with-opencode',
      commandNote: '需要时安装 OpenCode，安装 CrewBee，并校验安装结果。',
      copy: '复制',
      copied: '已复制',
      copyFailed: '复制失败',
      steps: [
